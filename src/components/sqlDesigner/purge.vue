  ...
  <v-flex id="mainflex">
    <v-card class="rounded-card">
        <!-- <v-layout row wrap ml-3 mr-3>
          <v-flex xs12 style="text-align:left">
            <span class="v-align">
              ...
              <span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;">{{purgeobj.diagram_name}} /</span>&nbsp;
        ...
        <span class="v-align" style="font-size:13px;float:right;color:rgba(0,0,255,1);margin-right:5px;">
          <showerrorlisttemplate @totalCount="totalCount" :validation_response="validation_response" stepType="Purge"
    ...
    <v-layout row wrap pr-4 mb-2  class="breadcrumbBackground">
      <v-flex xs9 pl-4 style="text-align:left;margin:auto">
        <ul class="breadcrumb" style="display:inline-flex;">
          ...
          <li><a @click="savedata(true)">{{purgeobj.diagram_name}} </a></li>
      ...
      <v-flex xs3>
        ...
        <span style="font-size:13px;float:right;margin-right:5px;" :style="{'color': colorCode}">
          ...
          <showerrorlisttemplate :validation_response="validation_response" stepType="Purge"
    ...
    <div id="join_panel" class="upperrow" style="width:79%;" ref="join_panel_ref">
      ...
      <v-checkbox :color="colorCode" label="Purge From Target" v-model="run_on_target"
      class="customCheckbox" @change="savedata()" hide-details>
    </v-checkbox>
  </div>  
</div> 
...
<v-layout raw wrap style="margin-left:1%;margin-right:1%">
 ...
 @savedata="saveCriteria" @new="add_new_filter_row" @delete="delete_filter_row" parentsteptype="purge"></criteria>     
</v-layout>
</v-card>
</v-flex>        
</template>
  ...
  export default {
    ...
    data () {
      return {
        ...
        run_on_target:false,
        limit:"100",
        stepname:this.purgeobj.cell_name,
        join_from_table:{},
        join_to_table:'',
        dateformatobj:'',
        jto_column_list:[],
    ...
    props:['purgeobj','bussinessObject','savedatastatus','iserroroccured','policies','bussinessObjectList','datasourceerror','tblsprimarykeys'],
    computed: {
      table_column_list(){
    ...
    watch: {
      '$store.state.errorList':{
        handler(newValue){
          if(!_.isEmpty(newValue)){
            ...
            if(this.purgeobj && newValue[this.purgeobj.stepId]){
              this.validation_response = newValue[this.purgeobj.stepId];
            }          
          }
        },
        deep:true
      ...
      if(flowchart_errors && this.purgeobj && flowchart_errors[newvalue.stepId]){
      ...
      methods:{
        ...
        savedata(is_close){
          let data = this.getPurgeStepData()
      ...
      validatestepdata(){
        ...
        var data = this.getPurgeStepData();
        ...
        validate_process_def_json(this, data).then(validation_response => {
          ...
          _this.$store.state.errorList[this.purgeobj.stepId] = validation_response
        },error=>{
         _this.validateProgress = false;
       })
      },
      ...
      var data = {'stepname':_this.stepname,'step_type':'Purge',
      'step_data':{"type":"Purge", "limit":this.limit,"isDriverFromDb":this.is_driver_is_from_db,"flattening_allowed":this.flattening_allowed,"run_on_target":this.run_on_target,
...
resetIsDrvTable(c_name, idx){
  ...
  setFilterColumn(){
    ...
    if(this.join_from_table.type === 'T')
      ...
      // let stepTable = find(_this.purgeobj.source_cells_detail, ['source_name', table]);
