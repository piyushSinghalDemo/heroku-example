  ...
  <v-flex id="mainflex">
    ...
    <v-layout row wrap pr-4 mb-2  class="breadcrumbBackground">
      ...
      <div style="width:99%;margin-left:7px;">
        <div id="table_list_panel" class="upperrow" style="width:20%;overflow: hidden;" ref="table_list_panel_ref">
          ...
          style="margin-top: 20px;"></v-autocomplete> 

          <v-btn :color="outlineColor"  outline style="border-radius:10px;float:right"  @click="GenerateBO" :loading="generateboclick"
        :disabled="generateboclick" v-if="is_driver_is_from_db">
           Generate BO
         </v-btn>
         <br v-if="is_driver_is_from_db">
         <br v-if="is_driver_is_from_db">

          <v-autocomplete clearable v-model="selected_bus_obj" label="Search Business object Here" 
          :items="bussinessObjectList" @input="setBussinessObject(selected_bus_obj)" item-text="name"  item-value="id" style="margin-top: 20px;"></v-autocomplete>                 
          <v-text-field label="Commit Size" v-model="limit" v-on:blur="savedata()"></v-text-field>

        
       <v-checkbox color="red" label="Flattening Allowed"  v-model="flattening_allowed"
    ...
    <div id="join_panel" class="upperrow" style="width:79%;" ref="join_panel_ref">
      <div id="archival_main_container" ref="archival_main_container" style="width:100%;height:450px;">
          ...
          <div id="selection_panel" class="upperrow" style="width:30%;float:left;margin-left:1%"> 
            <h6 style="margin-top:-10px;margin-left:5px;z-index:222;position:absolute;background:white;" >Driver Columns For Order By</h6>
            ...
            <h6 style="margin-top:-10px;margin-left:5px;z-index:222;position:absolute;background:white;" >Business Object Keys</h6>
            <span style="float:right;margin-top:-23px;margin-left:19%;position:absolute;background:white;">
            </span>
              <table class="table table-striped table-bordered draggable_table"  id="select_column_table" style="margin-top: 3%;">
                <thead>
...
const joint = require('jointjs')
  ...
  export default {
    ...
    props:['archivalobj','bussinessObject','savedatastatus','iserroroccured','policies','bussinessObjectList','datasourceerror'],
    computed: {
      table_column_list(){
    ...
    watch: {
      ...
      'join':[],'filter':filter_data,'order_by':this.columns_for_order
    },
...
resetIsDrvTable(c_name, idx){
  ...
  setFilterColumn(){
    ...
    GenerateBO(){
      ...
      var mapped_agent_id = _this.$session.get('selected_env').mapped_agent_id
      var schema = _this.schema
      var connection_str = _this.connection_str
      var table_name =_this.join_from_table.title
      var db_type =_this.db_type
      var client_id = this.$session.get('client_id')
      var added_by = this.$session.get('email')
      var data_to_pass = {"table_type":table_type,"env_id":env_id,"schema":schema,"connection_str":connection_str,"table_name":table_name,"db_type":db_type,"client_id":client_id,"env_name":env_name,"added_by":added_by,"object_type_code": "BO","mapped_agent_id":mapped_agent_id}
