<template>
  <v-flex id="mainflex">
      ...
      <!-- <div style="text-align: left;margin-left:1%;margin-right:1%"> -->
        <!-- <v-layout row wrap ml-3 mr-3>
          <v-flex xs12 style="text-align:left">
            <span class="v-align">
              <span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;"> Home /</span> &nbsp;<span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;">Process Designer /</span>&nbsp;<span style="color:rgba(0,0,0,0.54);font-weight:400;font-size:13px;">{{copyobj.diagram_name}} /</span>&nbsp;<span style="font-weight:400;font-size:15px;"><input type="text" v-model="stepname" disabled/></span><span style="font-weight:400;font-size:13px;margin-left:5px;" v-bind:class="{errorcls:is_error_occured , successcls:!is_error_occured}">{{'('+savedstatus + ')'}}</span><span style="font-weight:400;font-size:13px;margin-left:5px;" :style="{'color': colorCode}">{{datasource_error}}</span>
            </span>
            ...
            <span class="v-align" style="font-size:13px;float:right;color:rgba(0,0,255,1);margin-right:5px;">
              <showerrorlisttemplate @totalCount="totalCount" :validation_response="validation_response" stepType="Copy"
          ...
          <v-flex xs9 pl-4 style="text-align:left;margin:auto">
            <ul class="breadcrumb" style="display:inline-flex;">
              ...
              <li><a @click="savedata(true)">{{copyobj.diagram_name}} </a></li>
        ...
        <span style="font-size:13px;float:right;margin-right:5px;" :style="{'color': colorCode}">
              ...
              <showerrorlisttemplate :validation_response="validation_response" stepType="Copy"
    ...
    <div id="join_panel" class="upperrow" style="width:79%;" ref="join_panel_ref">
      ...
      <v-checkbox :color="colorCode" label="Copy From Target To Source" v-model="run_on_target"
      class="customCheckbox" @change="savedata()" hide-details>
    </v-checkbox>
  </div>  
</div> 
  ...
  export default {
    ...
    data () {
      return {
        ...
        limit:"100",
        stepname:this.copyobj.cell_name,
        join_from_table:{},
        join_to_table:'',
        dateformatobj:'',
        jto_column_list:[],
    ...
    props:['copyobj','bussinessObject','savedatastatus','iserroroccured','policies','bussinessObjectList','datasourceerror','tblsprimarykeys'],
    computed: {
      table_column_list(){
    ...
    watch: {
      '$store.state.errorList':{
        handler(newValue){
          if(!_.isEmpty(newValue)){
            ...
            if(this.copyobj && newValue[this.copyobj.stepId]){
              this.validation_response = newValue[this.copyobj.stepId];
            }          
          }
        },
        deep:true
      ...
      if(flowchart_errors && this.copyobj && flowchart_errors[newvalue.stepId]){
      ...
      validatestepdata(){
        ...
        validate_process_def_json(this, data).then(validation_response => {
          ...
          _this.$store.state.errorList[this.copyobj.stepId] = validation_response
        },error=>{
         _this.validateProgress = false;
       })
      },
      ...
      var data = {'stepname':_this.stepname,'step_type':'Copy',
      'step_data':{"type":"Copy", "limit":this.limit,"isDriverFromDb":this.is_driver_is_from_db,"flattening_allowed":this.flattening_allowed,"run_on_target":this.run_on_target,
...
resetIsDrvTable(c_name, idx){
  ...
  setFilterColumn(){
    ...
    if(this.join_from_table.type === 'T')
      ...
      // let stepTable = find(_this.copyobj.source_cells_detail, ['source_name', table]);
